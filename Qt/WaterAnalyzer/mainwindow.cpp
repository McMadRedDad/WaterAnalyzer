...
MainWindow::MainWindow(QWidget *parent)
  ...
  args << "-u" // "-u" for unbuffered stdout
       << "/home/tim/Учёба/Курсовая/code/python/server.py";
  backend->start("python3", args);
}
...
void MainWindow::prepare_backend() {
  ...
  connect(backend, &QProcess::started, this,
          [=] { append_log("info", "Процесс бэкенда запущен"); });
  connect(backend, &QProcess::finished, this,
          [=] { append_log("info", "Процесс бэкенда остановлен"); });
  connect(backend, &QProcess::errorOccurred, this, [=] {
    append_log(
        "bad",
        QString("Ошибка запуска бэкенда - %1").arg(backend->errorString()));
  });
  connect(backend, &QProcess::readyReadStandardError, this, [=] {
    append_log(
        "bad",
        QString("Ошибка на бэкенде: %1").arg(backend->readAllStandardError()));
  });
  connect(backend, &QProcess::readyReadStandardOutput, this,
          &MainWindow::backend_stdout);
}
...
void MainWindow::_connect_socket(QAbstractSocket *socket, QHostAddress address,
                                 quint16 port) {
  if (!socket) {
    return;
  }
  ...
  connect(socket, &QAbstractSocket::connected, this, [=] {
    ...
    append_log("good", QString("Успешное подключение к %1 по порту %2")
  ...
  connect(socket, &QAbstractSocket::disconnected, this, [=] {
    QAbstractSocket *sock = qobject_cast<QAbstractSocket *>(sender());
    append_log("info", QString("Подключение к %1:%2 закрыто")
                           .arg(sock->peerAddress().toString(),
                                QString::number(sock->peerPort())));
    sock = nullptr;
  });
...
void MainWindow::socket_error() {
  ...
  case QAbstractSocket::ConnectionRefusedError: {
    append_log("bad", "Критическая ошибка! Не удалось подключиться к бэкенду, "
  ...
  case QAbstractSocket::RemoteHostClosedError: {
    append_log(
        "info",
        QString("Бэкенд разорвал соединение: %1").arg(sock->errorString()));
    break;
  }
  default:
    append_log("bad", QString("Ошибка на %1:%2 - %3")
                          .arg(sock->peerAddress().toString(),
                               QString::number(sock->peerPort()),
                               sock->errorString()));
    break;
...
void MainWindow::closeEvent(QCloseEvent *e) {
  if (backend->state() == QProcess::Running) {
    append_log("info",
               "Бэкенд ещё работет, завершить программу сейчас невозможно");
    e->ignore();
  }
}
